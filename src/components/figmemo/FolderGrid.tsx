...
const FolderCard: React.FC<FolderCardProps> = ({
  ...
  const coverOverride = useFigmemoTagsStore((s) => s.folderCovers[folder.name]);
  const tagCount = useFigmemoTagsStore(
    (s) => s.getFolderTagIds(relPath).length,
  );
  const platform = folder.platform as PlatformSource | undefined;
  const setFolderCover = useFigmemoTagsStore((s) => s.setFolderCover);
  const { message } = App.useApp();
