...
- **详情「上一篇 / 下一篇」（本轮）**：详情页顶栏加「上一篇 / 下一篇」按钮 + `当前/总数` 进度。进入文章时把当时的筛选结果 `filtered` **快照**进 `navList`（`openPost(item, list)`），`下一篇/上一篇` 按快照顺序跳转（详情页 `filtered` 仍早退为 `[]`，**保留点标签/切文章不重算全量筛选的性能优化**）。打标时同步 `navList` 中该文章的 `articleTags`；切文章 `detailScrollRef` 回到顶部。用途：先筛「未打标签 + 年份」再逐篇打标，打过的文章即使不再命中筛选也仍在快照里，`下一篇` 继续沿原顺序走
- **快捷标签墙 + 空格下一篇（本轮）**：详情右下角标签浮窗由「每分类一个 `Select mode=tags`」改为**可点 chip 标签墙**——候选标签铺成一排，**点一下即打/取消**（免去点开下拉再选），已选高亮并在分类名旁显示计数；末尾「＋自定义」小输入回车新建（`addGroupTag`，值先乐观显示再落标签树）。**空格键 = 下一篇**（`Figmemo.tsx` 窗口 `keydown` 监听；`INPUT/TEXTAREA/SELECT/BUTTON/contentEditable` 内不触发，chip 点击后 `blur()` 防止空格二次激活按钮），`下一篇` 按钮加 `title="下一篇（空格键）"`
- **「多标签」退出行为修复（本轮）**：`components/figmemo/CategorySidebar.tsx` 的「多标签」开关原在退出时把多选收敛成最后一个标签继续筛选（易被当成「退出后筛选依旧生效」的 bug）。现改为**退出时清空 `tagIds`（回到全部）**，未打标签/收藏等其它条件保留；进入多标签时仍保留当前已选标签
- **Hpoi 辅助入口（本轮）**：详情页顶栏加带 hpoi 图标的「Hpoi」按钮，点击用浏览器打开**预填「商品名 + 厂商」的 hpoi 搜索页**（`https://www.hpoi.net/search?keyword=…&category=100`）。关键词 `buildHpoiKeyword(title, html)`：优先标题 `厂商「商品名」…`，正文 `メーカー：`/`商品名：` 兜底。图标 `src/assets/platform-icons/hpoi.png`（站点 favicon）。背景：hpoi **无公开 API**（只能解析 HTML / 详情页 JSON-LD），且本地化改名严重（メアリー→梅里、インプラカブル→怨仇），**纯日文名直搜不可靠**、厂商才是强消歧器；实测 fig-memo 最新 5 条 **5/5** 能在 hpoi 找到对应词条（靠商品名 + 厂商校验 + `alternateName` 复核）。故先做「预填搜索」零风险入口，不做脆弱的自动匹配
- **Hpoi 自动匹配评估结论（本轮，暂不做）**：30 条试跑——商品名直接搜 hpoi 仅 **43%**；按厂商扫目录 + `alternateName` 比对达 **66.7%（上限 ~76.7%）**，hpoi 收录率 **80%**，高置信精度 100%；可借字段覆盖率：评分/属性/比例/发售 100%、原型/原画 80%、角色/作品 25%。但成本：hpoi 手办分类实际 **~78,600 条**（详情全量 ≈34h），按厂商扫 **~65 请求/篇**（fig-memo 共 **2,633 篇** → 74h）；且 hpoi 列表**厂商筛选是 JS 动态加载**（`workers=`/`company=` 直抓 0 条目，仅 `/company/<id>` 带 pageCount）。结论：爬全量不现实也不必要；将来若做，应「fig-memo 驱动 + 按需匹配 + 永久缓存」并把单篇请求压到个位数。**用户决定暂不做。**（试跑脚本/报告在临时目录 `%TEMP%\opencode\figmemo_hpoi\`，非项目内）
- **【后续突破】反编译 hpoi APK 得到官方 JSON 接口**：用户提供 `base.apk`（包名 `net.hpoi`）。dex 内含完整 REST 路径清单，base = `https://www.hpoi.net/api/...`，**免登录、免签名**。核心 `GET /api/search?keyword=&page=1&pageSize=50` 返回 50 条/次，字段含 `name`/`name_ja`/`name_en`/`nameCN`/`aliases`（日文原名）、`companyName`/`companyId`、`scale`、`rating`/`ratingScore`/`commentCount`、`textTags`（标签）、`workerName`/`workerJob`（原型/原画）、`detail`、`cover`、`releaseDate`。完整日文名直搜：样本 205592→`111926` 第 2、205536→`117305` 第 1。→ 成本从 **~65 请求/篇 降到 1 请求/篇**，推翻「爬 HTML 成本爆炸」的旧结论。其它接口 `api/hobby/query-v2`(POST)、`api/company/v2/get`、`api/person/get`、`api/charactar/get`、`api/works/get`、`api/comment/get` 等（详情参数待摸，`search` 已够用）。⚠️ `robots.txt` 有 `Disallow: /api/`，属灰色地带，需限速+缓存、勿高频全量。脚本在 `%TEMP%\opencode\hpoi_apk\`。**待用户决定是否重跑 pilot / 落地。**
- **hpoi API pilot 结果（30 条）**：`GET /api/search?keyword=<厂商>` 取 `companyId`；`POST /api/hobby/query-v2` **必须 form-urlencoded**（JSON 被忽略），`workers`/`category`/`page`/`pageSize`/`order` 生效，可拉厂商全部条目。结果：厂商解析 **30/30**、目标在厂商目录（≤400）**18/24=75%**、naive 打分**选对 14/24=58%**、6 个未收录全误报（需阈值）。成本 **~3 请求/篇（冷，厂商目录缓存后≈0）**。瓶颈=**本地化改名**（APEX 205592 目标在目录第 105 位但 hpoi `name` 为中文无日文别名；厂商 id 解析也有错如 Lovely→良笑）。提升：翻页到底 + 打分加比例/发售/别名/词元 + 阈值 + 厂商纠错 + 半自动 top-3 确认。脚本 `%TEMP%\opencode\hpoi_api\`
- **Hpoi 候选关联（本轮，半自动第一步）**：详情页**右上角悬浮卡片**列 hpoi **top-3 候选**，打标时点一下关联；确认后写 `figmemo.jsonl` 的 `hpoi` 快照（名称/厂商/比例/评价/评论/`textTags`），已关联显示快照 + 「在 Hpoi 打开 / 解除」。流程（`src/services/hpoi.ts`）：`parseFigmemoFields`(标题`厂商「商品名」`/正文 `メーカー：`/`商品名：`/`スケール：`) → `resolveCompanyId`（`/api/search` 公司名相似度，缓存）→ `findHpoiCandidates`（`query-v2` form `workers+keyword`，空则回退全站 `/api/search`；打分=名称 dice×60 + 厂商别名×30 + 比例×10；top-3）。落地文件：`services/hpoi.ts`、`components/figmemo/HpoiMatchPanel.tsx`、`services/figmemo.ts`（`FigmemoMeta/FigmemoListItem` 加 `hpoi?`、`setHpoiMatch`、`buildItems`/`listLocalPosts` 带出）、`hooks/useRemoteImage.ts`（加 `{bypassProxy,headers}`，hpoi 封面直连 + `Referer` 破防盗链）、`pages/Figmemo.tsx`（`applyHpoiMatch` 同步 `selected`/`items`/`navList` + 落盘）。30 条实测预期 **top1 62% / top3 71%**；未命中用顶部「Hpoi」搜索按钮兜底。⚠️ 需 `pnpm tauri dev` 桌面实测。后续：新文只查近期窗口自动匹配；补完库可去掉面板只留数据。
- **Hpoi 手动校正 + JSON-LD 发现（本轮）**：面板加「**手动校正**」（贴 hpoi 链接/id → `parseHpoiId` → `fetchHpoiItemById` 关联）。**关键**：列表接口（`/api/search`、`query-v2`）**不含 `alternateName`** → 本地化条目自动匹配选不中；但**详情页 HTML 的 JSON-LD `Product` 有 `alternateName`（日文原名）+ 准确 `aggregateRating`**，且**必须浏览器 UA**（`hpoi/android` 会被降级成无 JSON-LD 的精简页）。`fetchHpoiItemById` 抓 `/hobby/<id>` 解析 JSON-LD（nameCN/日文别名/厂商/比例/评分 ratingValue/评分人数 ratingCount/封面/发售）；确认候选也走它取准确数据。后续可用「对 top 候选抓详情用 `alternateName` 复核」提升自动命中率。
- **hpoi 候选调优评估（用确认的 20 篇作真值，本轮）**：列表打分 top1 9/20、top3 11/20、top5 12/20，**启发式已到天花板**（试过并否决：搜索排名先验、详情 `alternateName` 重排前 8/前 20、纯搜索顺序、公司优先、厂商全目录）。根因：列表数据无日文别名 + 本地化改名 + 厂商名→companyId 解析不可靠。**已采纳**：`findHpoiCandidates` **始终并入 `/api/search`**（检索 12→14/20）；**比例硬过滤**（fig-memo 与 hpoi 比例都不缺且不一致的直接剔除，0/缺失当未知保留）；**「2+1」候选**——每批 2 条 = **hpoi 网站搜索页** `/search?keyword=<商品名 厂商>&category=100`（服务端渲染，**排序与浏览器一致**；`/api/search` 是另一套、常不对）解析 HTML + 1 条 = 厂商目录+`/api/search` 打分 top1（比例硬过滤）。**「重新查询」**= 排除当前已显示候选再取下一批（网页 +2、自研 +1）。**默认自动匹配第一条**：首次打开、初始批次、未关联时自动关联候选#1；错了点「重新检索」再手动挑。手动校正仍是可靠兜底。脚本 `%TEMP%\opencode\hpoi_api\pilot*.py`。
- **离线批量匹配（本轮）**：脚本 `%TEMP%\opencode\hpoi_api\batch_match.py`（可续跑、每 25 篇原子落盘）。取「hpoi 网站搜索页第一条」（标题解析 `厂商「商品名」` → 关键词 `商品名 厂商`；空则 `/api/search`），抓详情 JSON-LD 写 `figmemo.jsonl` 的 `hpoi`（已有不覆盖，无记录补建）。**2633 篇全部写入**（备份 `.bak-batchmatch`）。⚠️ 错配靠应用内「重新检索」纠正。
- **修复比例解析（本轮）**：`fetchHpoiItemById` 原正则把 `1/6` 解析成分子 `1`，改为取分母并兼容全角 `/／`；`parseScale` 同步。已回填 42 条（`.bak-scalefix`）。
- **fig-memo 列表排序 + 评分显示（本轮）**：工具栏加排序（组件内 state，不持久化）：日期上下 / **Hpoi 评分高低**；卡片加 `★评分`。数据来自批量写入的 `hpoi` 快照。
- **hpoi 匹配白名单分类（本轮）**：`HPOI_MATCH_CATEGORY_IDS=[16,140,364]`（フィギュアレビュー/レビュー(R18)/予約開始情報）；详情页仅当分类命中才渲染候选面板（其余不自动匹配、不请求）。`cleanup_cat.py` 已清掉其余分类（イベント×2/セール 共 65 篇）的 hpoi（备份 `.bak-catclean`），保留 2568 篇。
- **评分语义修复 + 绑定数据内置（本轮）**：① 列表接口 `/api/search` 的 `rating` 是总分/人数≠均分；`toCandidate` 不再带 `rating/commentCount`（只用详情 JSON-LD 均分），回填 2 条脏数据（117305→4.3、134338→4.38）。② 2568 条绑定导出 `src/data/hpoi-matches.json`（733KB）随包内置；`figmemo.ts` `resolveHpoi` 读时注入（本地 `hpoi` 优先 → 内置种子 → 除非 `hpoiRemoved`）；「解除关联」写 `hpoiRemoved` 防种子复活。前端包 1.60→2.16MB（gzip 511→734KB）。
- **修复 fig-memo 加载失败（XSERVER WAF，本轮）**：`/wp-json/` 被 XSERVER WAF 403（首页 200；与代理/节点/UA 无关）；`services/figmemo.ts` API 前缀改为 **`…/?rest_route=/wp/v2`**（WordPress rest_route 形式，WAF 不拦）+ 完整浏览器 UA。分类/详情/媒体均恢复 200。
- **修复「未下载显示 0 图」（本轮）**：`folderImageInfoCached`（一次 readDir 得本地首图+图片数，会话缓存）；`buildItems`/`listLocalPosts` 已下载用本地实际数；卡片仅 `imageCount>0` 显示「· N 图」；`openPost` 未下载拉图后也 `upsertMetaImageCount` 落库。未下载且未打开的文章暂无数字（可离线批量补）。
