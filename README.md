...
多平台内容下载器：**X（推特）** 媒体下载 + **Pawchive** 归档站（Patreon / Fanbox / Discord）检索、订阅与批量下载，支持统计、时间流、本地库。

> **本项目为 [X-Spider](https://github.com/MiningCattiva/x-spider) 的修改版（fork）**，在原版基础上新增订阅、统计、时间流、Pawchive 平台、本地库等功能。原版版权归其原作者所有，本项目遵循 GPL-3.0 许可证。
...
- **标签管理**：给作者 / 创作者文件夹打多个标签，左侧标签栏浏览（全部 / 未打标签 / 自定义标签）
- **本地浏览**：网格预览已下载内容；Pawchive 帖子目录支持「平铺 / 按文件夹」切换与多级下钻
- **属性**：查看文件夹的文件数、占用空间、标签
- **缩略图缓存**：图片自动生成小图缓存，二次浏览秒开（设置页可查看占用并一键清除）
- **排序**：按名称 / 日期 / 媒体数（文件夹）、名称 / 日期 / 类型（文件）
- **多选批量**：批量删除文件、批量添加 / 移除 / 清空标签
